...
export function ImageViewer({ item, onClose }: ImageViewerProps) {
  ...
  useEffect(() => {
    ...
    setFullSrc(null);
    setLoadError(false);
    if (!item.conversationId || !item.attachmentPath) return;
    const load = loadChatAttachmentImage(item.conversationId, item.attachmentPath);
    let cancelled = false;
    void load.promise.then((src) => {
  ...
  }, [item.attachmentPath, item.conversationId, item.src]);
