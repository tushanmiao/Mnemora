...
5. 不使用 `click`、外链图片或依赖宽松安全级别的语法；节点换行只允许使用安全的 `<br/>`，不要使用其他 HTML 标签。
...
11. 控制单图复杂度：线性链超过 6 个节点时优先考虑 `LR`；普通单图建议保留 12–18 个核心节点，巨型 ER 图按领域拆分为每图约 6–10 个实体，字段明细可移到表格或子图。
12. 不要用几十个节点组成一条纵向长链，也不要把全部数据库表及字段塞进一张 ER 图；概览图只保留关键关系，细节图按阶段或领域拆分。
13. 节点需要显式换行时使用 `<br/>`，不要输出裸 `<br>`；换行后仍应保持节点短语化，避免用 HTML 承载长段落。
