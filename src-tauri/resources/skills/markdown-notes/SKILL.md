...
version: 1.0.0
license: MIT
compatibility: 输出使用 Mnemora 当前可渲染的 CommonMark/GFM 子集，不依赖 Obsidian Vault、CLI、Wikilink 或专有嵌入语法。
triggers:
  - /markdown
  - /note
...
metadata:
  mnemora:
    ...
    adapted: true
    adaptation-notes: 将 Obsidian 专用 Markdown 工作流收敛为 Mnemora 可稳定渲染的 GFM；移除 Wikilink、Vault 嵌入、CSS class、Mermaid 和未支持语法。
---
...
- 不使用依赖特定笔记软件的 Wikilink、嵌入、CSS class 或脚本。
