...
## 13. 2026-08-21：模型能力、图片上限与运行预算可见性

本轮在上述安全合同上继续完成三类调整。

### 13.1 Chat 图片由 4 张提升到 10 张

- 单条 Chat 消息的附件总数统一为最多 10 个；
- 图片选择、会话持久化校验及实际模型请求均允许最多 10 张图片；
- 保留单图 10 MB、一次模型请求图片合计 16 MB、附件总计 25 MB 等既有字节安全边界；
- 增加“10 张图片可进入模型请求”和“第 11 个附件被拒绝”的 Rust 回归测试。

这里提升的是数量上限，不是总字节预算；因此不会因为允许更多小图而放宽大图导致的内存与请求体风险。

### 13.2 深度笔记显示模型 Tool、Vision 与 Skill 能力

深度笔记右侧“输入与模型”现在区分并展示：

- 模型 Tool 能力：`支持 / 不支持 / 未识别`；
- 模型视觉能力，并标记当前 Run 是否实际需要视觉；
- 模型推理能力；
- 本地 Reader 支持的 TXT、PDF、DOCX、XLSX 格式；
- 本次冻结的 Planner、Writer、Reviewer Skill 数量与名称。

Tool 能力改为三态，避免未知模型被错误显示为“不支持”。文案同时明确：文档附件由 Mnemora 的确定性本地 Reader 读取，并不等于开放了模型自由 Tool Loop。

### 13.3 语义预算与重试合同对齐

- 单章仍为最多 5 次新草稿尝试，加跨尝试累计最多 5 次语义修订；
- 初始语义预算按每章完整覆盖这 10 次有界任务计算；
- 图片视觉分析、尚未处理的来源分块、人工恢复和提纲调整会按需预留预算；
- 单 Run 语义调用硬上限由旧的局部 80/200 提升并统一为 640；
- 执行循环改为读取 Runtime 中的 `nodeAttemptLimit` 与 `sectionRevisionLimit`，避免显示值和真实行为漂移；
- 提纲调整正式执行 `replanLimit = 4` 并记录事件；
- UI 明确“重试 N 次”表示首次请求之外的 N 次网络重试，所以最多真实 HTTP 请求数是 `N + 1`，不与语义调用次数混为一谈。

### 13.4 来源分块上限仍为 96

单 Run 最多 96 个来源分块。该上限是对对话与附件分块合计生效的硬安全边界。本轮把检查前移到读取过程中：一旦下一块会超过上限就立即停止并给出明确错误，不再先把全部附件读完后才失败，也不会静默截断来源。

### 13.5 验证

- `cargo fmt --check`：通过；
- 深度笔记、附件、Chat 类型定向 Rust 测试：42 个通过；
- `cargo check`：通过，仅保留既有 dead-code warning；
- `npm test -- --run`：42 个测试文件、161 个测试通过；
- `npm run build`：通过；
- UI 静态质量检测：0 项问题；
- `git diff --check`：通过。
