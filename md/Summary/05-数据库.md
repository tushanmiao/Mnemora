...
Mnemora 不是一个 SQLite 数据库，而是两个独立的 SQLite 数据库文件：library.sqlite3 和 learning.sqlite3，分别形成两个事务域。library.sqlite3 内部又按业务职责划分为三个逻辑域：文献域、笔记域和深度笔记 Pipeline 运行域。这里的“三个域”不是“三个数据库”，而是同一个 Library 事务域内部的业务分区。除此之外，系统还使用会话 JSON、设置 JSON、Keyring 和附件目录，它们是不同的持久化介质，不应直接和 SQLite 事务域混为一谈。这个设计的核心是：逻辑上分域，事务上把需要原子更新的数据放在同一个 SQLite 文件中，物理上把高频学习事务与 Library 事务分开。
```mermaid
flowchart TB
...
stateDiagram-v2
    ...
    persisting --> done
    analyzing --> cancelling: 用户停止
    drafting --> cancelling: 用户停止
    persisting --> cancelling: 用户停止
    cancelling --> cancelled: 协作退出或强制终止
    analyzing --> paused
    drafting --> paused
    paused --> analyzing
    paused --> drafting
...
11. 后台深度笔记由 Supervisor 持有 JoinHandle/AbortHandle；取消请求、协作退出、强制终止和 panic 均写入有界任务诊断日志，避免“Token 已取消但 Run 永远 analyzing”。
