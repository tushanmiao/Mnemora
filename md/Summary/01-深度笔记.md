...
>
> 审计基线：提交 `eb3abfe`，应用版本 `0.1.16`，日期 2026-08-22。
>
...
| Evidence | 已打通但非 Claim 级 | 章节要求绑定 Source Chunk，不是逐句事实审查 |
| 提纲和人工确认 | 已打通 | JSON 校验、章节选择、最多 4 次调整 |
| 章节 DAG | 已打通 | 依赖、阻塞、检查点、恢复真实生效 |
| 真正并行章节生成 | 未打通 | Ready 可取 2 个，但仍逐个 `await` |
| 独立全局 Reviewer | 未打通 | `validate-global` 主要是状态转换 |
| 章节验证与修订 | 已打通 | 本地规则 + 最多 5 次语义修订 |
| Mermaid | 已打通 | 顶层图自动渲染；嵌套源码提供安全预览 |
...
| `evidence:*` | 有来源则 Verified；无来源则 Insufficient |
| `build-ledger` | 要求覆盖完整和真实 Ledger |
| `draft:*` | Writer 调用和章节检查点 |
| `validate:*` | 保存本地验证结果，不是独立模型 Reviewer |
| `validate-global` | 目前主要是本地状态转换 |
| `assemble-note` | 拓扑排序和 Markdown 组装 |
| `persist-note` | 笔记、来源、覆盖快照写库 |

章节 B 依赖 A 时，`draft:B` 等待 `validate:A`，并获得 A 的有限摘录。`maxParallelNodes=2` 目前只限制每轮取出的 Ready 数，实际仍按 `for ... await` 串行执行。
...
→ ReactMarkdown
→ MermaidBlock
→ Mermaid 11 串行 parse/render
→ SVG 清理、尺寸测量和大图查看器
```

每条消息最多 10 个 Mermaid，每块 24,000 字符；接近可视区域才渲染。历史嵌套 Mermaid 可以安全预览，不默认执行任意源码块。

### 11.2 公式

前端使用 `remark-math` 和 `rehype-katex`。Rust 把旧 `math/latex/tex` 围栏转为 `$$...$$`，最终使用 `$...$` 或 `$$...$$`。
...
详情页展示模型 Provider/名称、Tool 三态、Vision、Reasoning、本地 Reader、Skill Profile、Token 估算、分块进度、语义预算、模型活动、Source Chunk、Evidence、Ledger 和 DAG。
...
2. Ready 章节没有真正并行。
3. Source Recon/Evidence/Ledger 不是统一可重放 DAG Runner。
4. `validate-global` 没有独立跨章节语义审查。
5. 新附件增量 Recon 已有首版，但文献/笔记选区 Source Unit、Claim 级影响分析和跨 Run 解析缓存仍未完成。
6. 代码没有 AST、符号、import、调用图或项目 DAG。
7. Evidence 不是正文逐 Claim 验证。
8. 未确认提纲草案版本历史不完整。
...
优先顺序：Prompt 可观测性 → Source Unit → 附件增量 → 全局 Reviewer/Claim Evidence → 幂等输出 → 并行和缓存 → 项目级代码理解。

---

## 18. 验证基线

提交 `eb3abfe` 前后最近一次验证：Rust 307 个测试通过；前端 43 个测试文件、172 个测试通过；`cargo check`、`cargo fmt --check`、`npm run build` 和 `git diff --check` 通过。
...
它已经能说明读了哪些材料、用了哪些 Prompt/Skill、为何暂停或拒绝恢复、章节依赖如何释放和 Evidence 从哪里来；但还不能声称已经完成附件级增量、Claim 级验证、全局 Reviewer、真正并行、项目级代码理解或最终输出幂等事务。
...
2. maxParallelNodes=2 目前限制 Ready 数量，章节实际仍按 await 串行；
3. validate-global 主要是状态转换，不是独立跨章节 Reviewer；
