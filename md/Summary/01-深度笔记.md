...
> 2026-08-22 实施更新：进一步统一普通 Agent、上传层和深度笔记的附件格式注册表；代码文件获得独立 `code` 来源类型、语言元数据和“只读未执行”边界；敏感配置禁止自动进入深度笔记；新增附件不再陷入“增量拒绝、重建也拒绝”的死路，而是明确确认后按当前全部来源生成新笔记并保留旧笔记。详细记录见 `md/modify/05-深度笔记多格式附件与安全重建优化.md`。

---

## 0. 2026-08-21 当前实现增量

### 0.1 2026-08-22 多格式附件与安全重建增量

深度笔记仍使用四类确定性附件 Reader，但文本 Reader 现在由统一格式注册表覆盖多种文本、代码、配置、Schema 和特殊文件名；“四个 Reader”不再被误解为“四种扩展名”。

```mermaid
flowchart TB
    A[附件文件名、MIME、kind] --> R[统一格式注册表]
    R --> T[文本 / 代码 / 配置 Reader]
    R --> P[PDF Reader]
    R --> D[DOCX Reader]
    R --> X[XLSX Reader]
...
1. 普通 Agent、深度笔记和上传 MIME 识别共用 `chat/attachment_formats.rs`，消除扩展名白名单漂移。
2. `.py`、`.ts`、`.rs`、`.vue`、`.proto`、Dockerfile、Makefile 等代码/配置文件按完整行读取，并生成 `sourceKind=code` 的 Source Chunk。
3. 代码 Source Chunk 保留文件名、识别语言和行号，并明确“仅做静态文本读取，未执行代码”；当前不宣称 AST、调用图或仓库级语义。
4. 文本 Reader 根据 32 KB 字节预算计算实际完整行窗口；下一轮从 `actualEndLine + 1` 继续，不再从源码行中间截断或跳过未读后缀。单行本身超限时明确失败。
5. `.env`、凭据、Secret 等敏感文本不会自动送入深度笔记，即使普通文本 Reader 在技术上能够读取。
6. 已有笔记只有新增正文时继续走增量补丁；新增受支持附件时提示完整来源重建，创建新笔记并保留旧笔记，同时原子地把未来更新锚点切换到新笔记；新增不支持附件时列出具体文件并阻止无效重建。
7. 启动合同增加 `forceRebuild`，修复 `updateAvailable` 状态下增量拒绝附件、完整生成也被拒绝的流程死路。
8. 当前仍未实现附件增量 Evidence/Ledger 合并，因此“新增附件完整重建”是安全过渡策略；后续目标是附件级 Source Unit 与增量 Recon。
