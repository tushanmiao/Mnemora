...
> 2026-08-23 同步：内置 Skill 已切换为完整上游目录，原始 `SKILL.md`、references、scripts、assets 和许可证文件保持不变；Mnemora 仅通过同目录 `mnemora.json` 提供触发词、默认启用状态、Tool 映射和本地权限元数据。深度笔记过程进度继续由任务中心和数据库承载，输入区不再显示重复长驻进度 Toast。

> 文档性质：当前源码基线审计与使用说明，不把未来计划写成已实现功能。
>
> 审计基线：发布候选工作区，应用版本 `0.1.18`，日期 2026-08-22。
>
