...
- 当前数据库要求创建 Run 时为 1 到 5；
- 语义是“首次请求之外最多重试 N 次”；
- 因此默认 5 时，最多可能发送 6 次 HTTP 请求；
- 支持指数/策略等待和 `Retry-After`；
- CancellationToken 可以中断请求和退避等待。

当前实现还存在一个可直接由代码证明的启动缺陷：应用设置允许关闭自动重试；关闭后 `start()` 会把 `retry_attempts` 写成 `0`，表示“不进行网络重试”。但 `create_note_pipeline_run()` 创建 Run 时又要求该值必须位于 `1..=5`。因此，当用户关闭自动重试时，深度笔记很可能在创建 Run 阶段直接得到“深度笔记重试次数无效”，连第一次模型请求都还没有发出。
...
或者把 Run 字段正式允许为 `0..=5`。前一种方式更清楚，因为“用户配置的重试档位”和“本次生效的最大重试数”不是同一个字段。无论采用哪一种，都应增加“关闭自动重试仍能启动深度笔记”的 Store + Service 回归测试。
...
2. `Tool 可用` 当前展示的是模型能力快照，不代表深度笔记实际启用了 Tool，文案容易误导；
...
`replanLimit`、`replansUsed` 和 `Replanning` 阶段尚未形成触发、Plan Diff、影响分析和重新确认逻辑。

### 20.9 P1：Plan 草案版本历史不完整

多次“调整计划”应形成不可变的草案版本，并保存差异、原因、模型、Prompt Profile 和创建时间；当前主要持久化确认后的版本。

### 20.10 P1：预算字段与实际常量可能漂移

Runtime 保存 `nodeAttemptLimit` 和 `sectionRevisionLimit`，但执行循环直接使用编译期常量 `NODE_ATTEMPT_LIMIT` 和 `SECTION_REVISION_LIMIT`。未来若只修改数据库预算，执行行为不会跟着变化。
...
超过 96 个来源分块会拒绝执行。长期可增加用户可选范围、分层 Map-Reduce Ledger、主题聚类和增量计划，而不是无限提高上限。
