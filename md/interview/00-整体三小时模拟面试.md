...
深度笔记目前尚不是完整 Claim 级事实验证；Chunk Digest 和依赖就绪章节已实现默认并发 2 的有界执行，但 Provider 自适应背压、跨 Run 公平调度和增量附件 Digest 并行仍未完成；最终 `note_pipeline_outputs` 幂等闭环有缺口。代码文件目前是静态带行号读取，没有 AST/调用图；PDF 扫描/OCR/复杂版面仍需低置信度降级；IELTS Band 估计和官方题库校准未完成；内存 Benchmark/soak 需持续建设；插件执行型能力不应在第一阶段开放。明确缺口本身是可信度的一部分。
...
代表当前单 Run 内最多两个依赖就绪章节会真实并行执行模型计算，但不代表端到端所有阶段都并行。Worker 使用不可变 Job，Coordinator 单点提交 Scheduler、Runtime、SQLite 和事件；批次启动前悲观预留语义预算，完成后归还未使用修订额度。附件 Reader 仍受控串行，Provider 429/延迟尚未动态降低并发，跨进程原子领取仍需 Lease，因此配置是明确的本地并发合同，不是无限吞吐承诺。

### 追问：Chunk 并行为什么不能只保存“已完成 7/10”？

并发没有稳定前缀，7/10 无法说明具体完成了哪 7 块。当前每个 Chunk Digest 独立保存，并用 contentHash、promptHash、provider/model 判定是否可复用；Worker 乱序完成，Coordinator 按原 Chunk 顺序归并 Ledger。这样恢复只重做缺失或失效块，并发不会改变语义顺序。
...
面试最后要求候选人按“地基—能力—体验”排序，推荐顺序是：Source/Evidence 与最终幂等事务 → Snapshot/恢复/指标 → Claim/Citation 与 AST → 自适应并发背压和更丰富图形 → 插件生态与高级 Band 估计。
...
4. 选择一个尚未完成能力（Claim 级证据、AST、OCR、Provider 自适应并发或 FSRS Band 校准），写出四周里程碑、指标、回滚开关和不做的事情。
